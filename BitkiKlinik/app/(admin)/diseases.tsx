...
const LIGHT_C = {
  ...
  border: '#e2e8f0', cardBg: '#fff',
};

const DARK_C = {
  primary: '#818cf8', emerald: '#10b981', emeraldLight: '#064e3b',
  danger: '#f87171', dangerLight: '#7f1d1d', slate: '#f8fafc',
  slateLight: '#94a3b8', bg: '#0f172a', white: '#1e293b',
  border: '#334155', cardBg: '#1e293b',
};
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={s.header}>
          ...
          <TouchableOpacity style={s.addBtn} onPress={() => setShowModal(true)}>
            <Ionicons name="add" size={22} color={LIGHT_C.white} />
          </TouchableOpacity>
        </View>
        ...
        <Modal visible={showModal} animationType="slide" transparent>
          <View style={s.mOverlay}>
            <View style={s.mContent}>
              ...
              <TextInput style={s.input} placeholder="Hastalık adı" placeholderTextColor={C.slateLight} value={form.name}
                onChangeText={(v) => setForm({ ...form, name: v })} />
              <TextInput style={s.input} placeholder="Açıklama" placeholderTextColor={C.slateLight} multiline value={form.description}
                onChangeText={(v) => setForm({ ...form, description: v })} />
              <TextInput style={s.input} placeholder="Model etiketi (ör: Tomato__Blight)" placeholderTextColor={C.slateLight} value={form.modelLabel}
...
const getStyles = (C: any) => StyleSheet.create({
  ...
  backBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: C.cardBg, justifyContent: 'center', alignItems: 'center', elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
  ...
  card: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.cardBg, padding: 16, borderRadius: 16, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 1 },
  ...
  mContent: { backgroundColor: C.cardBg, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 },
  mTitle: { fontSize: 20, fontWeight: 'bold', color: C.slate, marginBottom: 20 },
  input: { backgroundColor: C.bg, color: C.slate, borderRadius: 12, padding: 14, fontSize: 15, marginBottom: 12, borderWidth: 1, borderColor: C.border },
  ...
  submitTxt: { fontSize: 15, fontWeight: '700', color: LIGHT_C.white },
});
